...
//---- START CONVENIENCE METHODS --------------------------------------
        ...
        public static function exportRoot(vars:Object = null, omitDelayedCalls:Boolean = true):TimelineLite {
            ...
            var tl:TimelineLite = new TimelineLite(vars), root:SimpleTimeline = tl._timeline;
...
//---- END CONVENIENCE METHODS ----------------------------------------
        ...
        override public function add(value:*, position:* = "+=0", align:String = "normal", stagger:Number = 0):* {
            ...
            if (!(value is Animation)) {
                if (value is Array) {
                    var i:int, curTime:Number = Number(position), l:Number = value.length, child:*;
                    for (i = 0; i < l; i++) {
                        child = value[i];
                        if (child is Array) {
            ...
            if (_gc || _time === _duration)
                if (!_paused)
                    if (_duration < duration()) {
                        ...
                        var tl:SimpleTimeline = this, beforeRawTime:Boolean = (tl.rawTime() > value._startTime); //if the tween is placed on the timeline so that it starts BEFORE the current rawTime, we should align the playhead (move the timeline). This is because sometimes users will create a timeline, let it finish, and much later append a tween and expect it to run instead of jumping to its end state. While technically one could argue that it should jump to its end state, that's not what users intuitively expect.
                        while (tl._timeline) {
        ...
        override public function render(time:Number, suppressEvents:Boolean = false, force:Boolean = false):void {
            ...
            var totalDur:Number = (!_dirty) ? _totalDuration : totalDuration(), prevTime:Number = _time, prevStart:Number = _startTime, prevTimeScale:Number = _timeScale, prevPaused:Boolean = _paused, tween:Animation, isComplete:Boolean, next:Animation, callback:String, internalForce:Boolean;
            if (time >= totalDur) {
        ...
        public function getChildren(nested:Boolean = true, tweens:Boolean = true, timelines:Boolean = true, ignoreBeforeTime:Number = -9999999999):Array {
            var a:Array = [], tween:Animation = _first, cnt:int = 0;
            while (tween) {
        ...
        public function getTweensOf(target:Object, nested:Boolean = true):Array {
            var disabled:Boolean = this._gc, a:Array = [], cnt:int = 0, tweens:Array, i:int;
            if (disabled) {
        ...
        override public function _kill(vars:Object = null, target:Object = null):Boolean {
            ...
            var tweens:Array = (target == null) ? getChildren(true, true, false) : getTweensOf(target), i:int = tweens.length, changed:Boolean = false;
            while (--i > -1) {
        ...
        public function clear(labels:Boolean = true):* {
            var tweens:Array = getChildren(false, true, true), i:int = tweens.length;
            _time = _totalTime = 0;
            while (--i > -1) {
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------------------------------------
        ...
        override public function totalDuration(value:Number = NaN):* {
            if (!arguments.length) {
                if (_dirty) {
                    var max:Number = 0, tween:Animation = _last, prevStart:Number = Infinity, prev:Animation, end:Number;
                    while (tween) {
